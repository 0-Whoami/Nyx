...
public class FileUtils {
    ...
    public static boolean isPathInDirPaths(String path, final List<String> dirPaths, final boolean ensureUnder) {
        if (path == null || path.isEmpty() || dirPaths == null || dirPaths.isEmpty())
            return false;
        try {
    ...
    public static Error validateDirectoryFileEmptyOrOnlyContainsSpecificFiles(String label, String filePath, final List<String> ignoredSubFilePaths, final boolean ignoreNonExistentFile) {
        ...
        if (filePath == null || filePath.isEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(label + "file path", "isDirectoryFileEmptyOrOnlyContainsSpecificFiles");
        try {
     ...
     */
    public static void copyOrMoveFile(String label, final String srcFilePath, final String destFilePath, final boolean moveFile, final boolean ignoreNonExistentSrcFile, int allowedFileTypeFlags, final boolean overwrite, final boolean overwriteOnlyIfDestSameFileTypeAsSrc) {
        label = (label == null || label.isEmpty() ? "" : label + " ");
        if (srcFilePath == null || srcFilePath.isEmpty()) {
            FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(label + "source file path", "copyOrMoveFile");
            return;
        }
        if (destFilePath == null || destFilePath.isEmpty()) {
            FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError(label + "destination file path", "copyOrMoveFile");
            return;
        }
        ...
        try {
            ...
            if (srcFileCanonicalPath.equals(destFileCanonicalPath)) {
                FileUtilsErrno.ERRNO_COPYING_OR_MOVING_FILE_TO_SAME_PATH.getError(mode + " " + label + "source file", srcFilePath, destFilePath);
                return;
            }
            ...
            if (destFileType != FileType.NO_EXIST) {
                ...
                if (overwriteOnlyIfDestSameFileTypeAsSrc && destFileType != srcFileType) {
                    FileUtilsErrno.ERRNO_CANNOT_OVERWRITE_A_DIFFERENT_FILE_TYPE.getError(label + "source file", mode.toLowerCase(), srcFilePath, destFilePath, destFileType.getName(), srcFileType.getName());
                    return;
                }
    ...
    public static void setFilePermissions(final String filePath, final String permissionsToSet) {
        ...
        File file = getFile(filePath, permissionsToSet);
    ...
    @NonNull
    private static File getFile(String filePath, String permissionsToSet) {
        File file = new File(filePath);
    ...
    public static Error checkMissingFilePermissions(String label, final String filePath, final String permissionsToCheck, final boolean ignoreIfNotExecutable) {
        ...
        if (permissionsToCheck.contains("r") && !file.canRead()) {
            return FileUtilsErrno.ERRNO_FILE_NOT_READABLE.getError(label + "file", filePath).setLabel();
        }
        // If file is not writable
        if (permissionsToCheck.contains("w") && !file.canWrite()) {
            return FileUtilsErrno.ERRNO_FILE_NOT_WRITABLE.getError(label + "file", filePath).setLabel();
        ...
        if (permissionsToCheck.contains("x") && !file.canExecute() && !ignoreIfNotExecutable) {
            return FileUtilsErrno.ERRNO_FILE_NOT_EXECUTABLE.getError(label + "file", filePath).setLabel();
        }
        return null;
    }
