...
public final class AppShell {
     ...
     */
    public static AppShell execute(@NonNull final Context currentPackageContext, @NonNull ExecutionCommand executionCommand, final AppShellClient appShellClient, @NonNull final IShellEnvironment shellEnvironmentClient, @Nullable HashMap<String, String> additionalEnvironment, final boolean isSynchronous) {
    ...
    private void executeInner() throws IllegalThreadStateException, InterruptedException {
        ...
        StreamGobbler STDOUT = new StreamGobbler(mProcess.getInputStream(), mExecutionCommand.resultData.stdout);
        StreamGobbler STDERR = new StreamGobbler(mProcess.getErrorStream(), mExecutionCommand.resultData.stderr);
        // start gobbling
        STDOUT.start();
        STDERR.start();
