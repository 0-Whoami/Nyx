...
public class PermissionUtils {
     ...
     */
    public static void requestPermission(@NonNull Context context, @NonNull String permission, int requestCode) {
     ...
     *                    will fail silently and will log an exception.
     */
    public static void requestPermissions(@NonNull Context context, @NonNull String[] permissions, int requestCode) {
        List<String> permissionsNotRequested = getPermissionsNotRequested(context, permissions);
        if (!permissionsNotRequested.isEmpty()) {
            return;
        }
     ...
     */
    public static boolean checkAndRequestLegacyOrManageExternalStoragePermission(@NonNull Context context, int requestCode) {
    ...
    public static boolean checkStoragePermission(@NonNull Context context, boolean checkLegacyStoragePermission) {
        if (checkLegacyStoragePermission || Build.VERSION.SDK_INT < Build.VERSION_CODES.R) {
            return checkPermissions(context, new String[] { Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE });
        } else {
    ...
    public static void requestManageStorageExternalPermission(@NonNull Context context, int requestCode) {
        ...
        if (requestCode >= 0)
            ...
            error = ActivityUtils.startActivity(context, intent);
    ...
    public static boolean isLegacyExternalStoragePossible(@NonNull Context context) {
        return !(Build.VERSION.SDK_INT >= Build.VERSION_CODES.R && PackageUtils.getTargetSDKForPackage(context) >= Build.VERSION_CODES.R);
    }
    ...
    public static boolean hasRequestedLegacyExternalStorage(@NonNull Context context) {
        ...
        return hasRequestedLegacyExternalStorage == null || hasRequestedLegacyExternalStorage;
    }
