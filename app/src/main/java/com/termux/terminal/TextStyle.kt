...
object TextStyle {
    ...
    fun encode(foreColor : Int, backColor : Int, effect : Int) : Long {
        ...
        result = if (-0x1000000 == (-0x1000000 and foreColor))  // 24-bit color.
        ...
        result = if (-0x1000000 == (-0x1000000 and backColor))  // 24-bit color.
    ...
    fun decodeForeColor(style : Long) : Int =
        if (0L == (style and CHARACTER_ATTRIBUTE_TRUECOLOR_FOREGROUND.toLong())) ((style ushr 40) and 511L).toInt()
        else -0x1000000 or ((style ushr 40) and 0x00ffffffL).toInt()


    fun decodeBackColor(style : Long) : Int =
        if (0L == (style and CHARACTER_ATTRIBUTE_TRUECOLOR_BACKGROUND.toLong())) ((style ushr 16) and 511L).toInt()
        else -0x1000000 or ((style ushr 16) and 0x00ffffffL).toInt()
