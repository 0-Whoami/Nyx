...
public final class TerminalColors {
    ...
    private static int parse(String c) {
        try {
            ...
            int charsForColors = c.length() - skipInitial - 2 * skipBetween; // Unequal lengths.
            if (0 != charsForColors % 3) return 0;
            int componentLength = charsForColors / 3;
            double mult = 255 / (Math.pow(2, (componentLength >> 2)) - 1);
    ...
    public void tryParseColor(int intoIndex, String textParameter) {
        int c = TerminalColors.parse(textParameter);
