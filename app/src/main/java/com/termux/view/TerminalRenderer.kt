...
class TerminalRenderer(textSize : Int) {
    ...
    fun render(mEmulator : TerminalEmulator, canvas : Canvas, topRow : Int) {
        ...
        for (row in topRow until endRow) {
            ...
            if (row in selectors[1]..selectors[3]) {
                ...
                selx2 = if (row == selectors[3]) selectors[2] else mEmulator.mColumns
            }
            ...
            while (column < columns) {
                ...
                if (style != lastRunStyle || insideCursor != lastRunInsideCursor || insideSelection != lastRunInsideSelection || fontWidthMismatch || lastRunFontWidthMismatch) {
                    if (0 != column) {
                        ...
                        val cursorColor = if (lastRunInsideCursor) mEmulator.mColors.mCurrentColors[COLOR_INDEX_CURSOR] else 0
                        val invertCursorTextColor = lastRunInsideCursor && TerminalEmulator.TERMINAL_CURSOR_STYLE_BLOCK == cursorShape
                        drawTextRun(canvas,
                                    line,
                                    palette,
                                    heightOffset,
                ...
                while (currentCharIndex < charsUsedInLine && 0 >= width(line,
                                                                        currentCharIndex)) { // Eat combining chars so that they are treated as part of the last non-combining code point,
    ...
    private fun drawTextRun(canvas : Canvas,
        ...
        if (-0x1000000 != (foreColor and -0x1000000)) { // If enabled, let bold have bright colors if applicable (one of the first 8):
        ...
        if (0 != cursor) {
            mTextPaint.color = cursor // fontLineSpacingAndAscent - fontAscent isn't equals to
            // fontLineSpacing?
